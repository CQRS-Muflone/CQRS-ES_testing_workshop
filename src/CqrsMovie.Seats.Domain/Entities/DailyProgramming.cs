...
namespace CqrsMovie.Seats.Domain.Entities
{
    public class DailyProgramming : AggregateRoot
    {
        ...
        internal void BookSeats(DailyProgrammingId aggregateId, IEnumerable<Messages.Dtos.Seat> seatsToBook)
        {
            ...
            {
                RaiseEvent(new SeatsAlreadyTaken(aggregateId, seatsToBook));
                return;
            }
        ...
        #region UnReserveSeats
        internal void UnReserveSeat(DailyProgrammingId aggregateId, IEnumerable<Messages.Dtos.Seat> seatsToReserve)
        {
            // Chk for seats availability
            var seatsToChk = seats.Where(seat => seatsToReserve.Any(book => book.ToEntity(SeatState.Reserved).Equals(seat)));
            if (seatsToChk.Count() != seatsToReserve.Count())
                throw new Exception("");

            RaiseEvent(new SeatsUnReserved(aggregateId, seatsToReserve));
        }

        private void Apply(SeatsUnReserved @event)
        {
            foreach (var seatReserved in @event.Seats)
            {
